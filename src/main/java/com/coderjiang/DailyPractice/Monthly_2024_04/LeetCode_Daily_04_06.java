...
//            }
        ...
        public int getKthAncestor(int node, int k) {
            return getAncestry(nodes, node, k);
        }

        private int getAncestry(List<SkipTreeNode> nodes, int curNode, int k) {
//            System.out.println("curNode = " + curNode);
//            System.out.println("k = " + k);
            Map<Integer, SkipTreeNode> curNodeFathers = nodes.get(curNode).fathers;
            Set<Integer> skipSteps = curNodeFathers.keySet();
            int maxAllowStep = -1;
            for (Integer skipStep : skipSteps) {
                if (skipStep <= k && skipStep > maxAllowStep) {
                    maxAllowStep = skipStep;
                }
            }
//            System.out.println("maxAllowStep = " + maxAllowStep);
            // 表示没有符合要求的祖先节点
            if (maxAllowStep == -1) return -1;
            if (maxAllowStep < k) return getAncestry(nodes, curNodeFathers.get(maxAllowStep).val, k - maxAllowStep);
            return curNodeFathers.get(maxAllowStep).val;
        }

        private void buildSkipLinks(List<SkipTreeNode> nodes, boolean[] isLeaf) {
            int skipStep = 2;
            while (skipStep <= MAX_NODES) {
                for (int i = 0; i < isLeaf.length; i++) {
                    if (isLeaf[i]) {
                        int curNode = i;
                        int ancestryNode = -1;
                        do {
                            ancestryNode = getAncestry(nodes, curNode, skipStep);
//                            System.out.println("curNode = " + curNode);
//                            System.out.println("ancestryNode = " + ancestryNode);
                            if (ancestryNode != -1) {
                                nodes.get(curNode).fathers.put(skipStep, nodes.get(ancestryNode));
                                curNode = ancestryNode;
                            }
                        } while (ancestryNode != -1);
                    }
                }
                skipStep <<= 1;
            }
